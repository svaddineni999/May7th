...
angular.module('datapackshome', ['vlocity', 'datapackshome.core', 'ngTable', 'ngSanitize', 'drvcomp', 'sldsangular', 'infinite-scroll'])
...
angular.module('datapackshome')
    .controller('homeController', function($scope, $rootScope, $sldsModal, $localizable,
        ...
        $rootScope.showInfo = $scope.showInfo = function(datapack, org) {
            ...
            modalScope.datapack = JSON.parse(JSON.stringify(datapack));
...
angular.module('datapackshome')
    .controller('installedController', function($scope, $sldsModal, $sanitize, $timeout, drvImportScopeManager,
                                            remoteActions, $filter, $rootScope, IsFoundation) {
        'use strict';
        $scope.orgs = [];
...
angular.module('datapackshome')
    .controller('publishedController', function($scope, $sldsModal, $sanitize, drvImportScopeManager, $timeout,
                                            remoteActions, $filter, drvDataPack, $q, $rootScope, IsFoundation) {
        'use strict';
        ...
        // Hide table features when OmniStudio is enabled
        if (IsFoundation) {
            // Removes the Published and Update columns
            $scope.publishedTableParams.columns.splice(4, 2);

            // Removes the Publish, Unpublish, and Update actions in DataPacks table
            $scope.publishedTableParams.actions.splice(2, 3);
        }
