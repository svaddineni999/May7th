...
angular.module('vlocity-oui-common')
    .factory('uiStateService', ['$rootScope', '$injector', function($rootScope, $injector) {
        ...
        else if(!window.OmniOut)
        {
            ...
            window.forceTKClient.setSessionToken(window.sessionId, 'v62.0', instanceURL);
        }
                ...
                else if($scope.bpTree.children && $scope.bpTree.children.length > 0)
                {
                    ...
                    if (bpService.scriptState === 'new') {
                        var configObj = {
                            ...
                            componentName: 'AngularRuntime_' + $scope.bpTree.bpType + '_' + $scope.bpTree.bpSubType + '_' + $scope.bpTree.bpLang,
                            componentId: $scope.bpTree.sOmniScriptId,
                        }
