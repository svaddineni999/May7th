...
angular.module('drvcomp')
    .config(['remoteActionsProvider', 'fileNsPrefixDot', function(remoteActionsProvider, fileNsPrefixDot) {
        ...
        var remoteActions = {
            ...
            getFileBasedOmniScripts: {
                action: fileNsPrefixDot() + 'OmniScriptHomeController.getFileBasedOmniScripts',
                config: {escape: false,  buffer: false, timeout: 120000}
            },
            getFileBasedIntegrationProcedures: {
                action: fileNsPrefixDot() + 'OmniScriptHomeController.getFileBasedIntegrationProcedures',
                config: {escape: false,  buffer: false, timeout: 120000}
            },
            getFileBasedDataRaptors: {
                action: fileNsPrefixDot() + 'DRDataPackRunnerController.getFileBasedDataRaptors',
                config: {escape: false,  buffer: false, timeout: 120000}
            }
        };
    ...
    .filter('readableDataPackTypeName', function(fileNsPrefix) {
        ...
        return function(objectType) {
            if (objectType === 'MultiPackCombination') {
                ...
                } else if (objectType === 'DataRaptor') {
                    objectType = 'Data Mapper';
                }
