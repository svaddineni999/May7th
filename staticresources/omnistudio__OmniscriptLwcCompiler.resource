...
if (typeof Visualforce !== 'undefined' && typeof angular !== 'undefined') {
...
require('./modules/omni-lwc-compiler/compiler/lwc/compiler.js');
},{"./dependencies/angular-drag-and-drop-lists.js":2,"./dependencies/native.history.js":3,"./modules/omni-lwc-compiler/compiler/lwc/compiler.js":7,"./modules/omni-lwc-compiler/config/config.js":23,"./modules/omni-lwc-compiler/services/bpService.js":24,"./modules/omni-lwc-compiler/services/compilerService.js":25,"./modules/omni-lwc-compiler/services/toolingService.js":26,"./modules/omni-lwc-compiler/vfPageController.js":27,"./polyfills/Array.find.js":29,"./polyfills/Array.findIndex.js":30,"./polyfills/Array.from.js":31,"./sharedObjectService.js":32}],2:[function(require,module,exports){
/**
...
const OmniscriptLwcSldsTemplate = require('../templates/managed/sldsTemplate');
const OmniscriptLwcNdsTemplate = require('../templates/managed/ndsTemplate');
const OmniscriptLwcCssTemplate = require('../templates/managed/cssTemplate');
const OmniscriptLwcNdsCssTemplate = require('../templates/managed/ndsCssTemplate');
const DeactivatedOmniscriptLwcHtmlTemplate = require('../templates/deactivated/htmlTemplate');
const DeactivatedOmniscriptLwcCssTemplate = require('../templates/deactivated/cssTemplate');
const CoreRuntimeOmniscriptLwcHtmlTemplate = require('../templates/coreruntime/htmlTemplate');
const CoreRuntimeOmniscriptLwcCssTemplate = require('../templates/coreruntime/cssTemplate');
const editBlockTemplates = require('../templates/managed/editBlockTemplate');
const OmniscriptLwcCompilerConfig = require('../compilerConfig.js');

let minify = function (input) {
    return Promise.resolve(input);
}

if (typeof Worker === 'function' && window.minifierWorker) {
    window.minifierWorker.onmessage = function() {
        minify = function (value) {
            return new Promise((resolve, reject) => {
                window.minifierWorker.onmessage = function(event) {
                ...
                window.minifierWorker.postMessage({
                    value: value,
                    options: {
                        "caseSensitive": false,
...
function OmniscriptHtmlBuilder(namespacePrefix) {
  ...
  const allowedUiElements = new Set([
    ...
    'Telephone', 'Text', 'Text Area', 'Time', 'Type Ahead Block', 'URL', 'Type Ahead', 'Places Typeahead', 'File', 'Image'
  ]);
  const allowedAggElements = new Set(['Aggregate', 'Formula', 'Custom Lightning Web Component']);
  const rootAppliedElements = new Set(['DocuSign Signature Action', 'DocuSign Envelope Action']);
  ...
  this.getHtml = function (json) {
    ...
    if (editBlockTemplates.editBlockReplace) {
      for (let ebTmpl of Object.keys(editBlockTemplates.editBlockReplace)) {
        sldsRootTemplate = sldsRootTemplate.replace(new RegExp(ebTmpl, 'g'), editBlockTemplates.editBlockReplace[ebTmpl].slds);
      }
    }
    ...
    if (editBlockTemplates.editBlockReplace) {
      for (let ebTmpl of Object.keys(editBlockTemplates.editBlockReplace)) {
        ndsRootTemplate = ndsRootTemplate.replace(new RegExp(ebTmpl, 'g'), editBlockTemplates.editBlockReplace[ebTmpl].nds);
      }
    }
    ...
    ndsRootTemplate = ndsRootTemplate.replace(/\{\$stepChart\}/g, stepChart)
      ...
      return minify(sldsRootTemplate).then(slds => {
          return Promise.all([
              Promise.resolve(slds),
              minify(ndsRootTemplate)
          ]);
      }).then(results => {
          return {
              slds: results[0],
  ...
  this.processJson = function (json, isDesignMode) {
    json.children = json.children.filter(child => child.type !== 'OmniScript');
    getUniqueComponents(json.children, new Map(), json.labelMap, json.propSetMap.elementTypeToLwcTemplateMapping || {}, json.rMap, isDesignMode, json);
  }

  this.getCoreRuntimeHtml = function (type, subType, language) {
    return {
        html: CoreRuntimeOmniscriptLwcHtmlTemplate
        .replace(/\{\$namespace\}/g, namespacePrefix)
        .replace(/\{\$subType\}/g, subType)
        .replace(/\{\$type\}/g, type)
        .replace(/\{\$language\}/g, language),
      css: CoreRuntimeOmniscriptLwcCssTemplate
    };
  }
  ...
  function buildOmniHtml(json) {
    ...
    getUniqueComponents(json.children, uniqueComponents, json.labelMap, json.propSetMap.elementTypeToLwcTemplateMapping || {}, json.rMap, null, json);
   ...
   * @param {Object} rootJson The root jsonDef of the OmniScript.
   */
  function getUniqueComponents(componentChildren, components, labelMap, globalLwcOverride, rMap, isDesignMode, rootJson) {
    componentChildren.forEach((child, index) => {
      if (child.type === 'Navigate Action' && child.name === 'CANCEL') {
        child.type = 'Cancel Action';
      }

      if (child.type === 'Navigate Action' && child.propSetMap.targetType === 'Restart OmniScript') {
        rootJson.hasRestartAction = true;
      }
      ...
      getUniqueChildComponents([index], child, componentConfig.children, 1, labelMap, `${child.name}`, `${child.name}`, child.propSetMap.uiElements, globalLwcOverride, rMap, isDesignMode, child.propSetMap.aggElements, rootJson);
   ...
   */
  function getUniqueChildComponents(parentIndex, component, uniqueComponents, currentLevel, labelMap, jsonPath, labelPath, uiElements, globalLwcOverride, rMap, isDesignMode, aggElements, rootJson) {
    ...
    component.children.forEach((child, childIndex) => {
      ...
      child.eleArray.forEach((element, elementIndex) => {
        ...
        if (element.type === 'Navigate Action' && element.propSetMap.targetType === 'Restart OmniScript') {
          rootJson.hasRestartAction = true;
        }
        ...
        if (element.children && element.children.length > 0) {
          ...
          getUniqueChildComponents(newParentIndex, element, componentConfig.children, newLevel, labelMap, newJsonPath, newLabelPath, uiElements, globalLwcOverride, rMap, isDesignMode, aggElements, rootJson);
        ...
        if (element.type === 'Edit Block') {
          ...
          child.sldsCls = editBlockTemplates.editBlockSldsTemplateCls.replace(/\$CTRLWIDTH/g, width);
          child.ndsCls = editBlockTemplates.editBlockNdsTemplateCls.replace(/\$CTRLWIDTH/g, width);
        ...
        if (eleType === 'Custom Lightning Web Component' && element.propSetMap.lwcName && element.propSetMap.lwcName.length > 0) {
          ...
          element.propSetMap.customAttributes.forEach(attr => {
            ...
            if (attr.name && typeof attr.source === 'string' && !custom.omniattributes[attr.name]) {
              let mergeField = attr.source;
        ...
        // Original, which includes labelPath
        labelMap[element.name] = `${labelPath}:${element.name}`;

        // Reassign element's label map value to only include element name if element is applied at the root
        if (rootAppliedElements.has(element.type) === true) {
          labelMap[element.name] = element.name;
        }
  ...
  function addLwcVariableList(json, uniqueComponent) {
    ...
    uniqueComponent.forEach(component => {
      if (component.children && component.children.size > 0) {
        // custom lwc directly under step
        component.children.forEach(childComponent => {
          if (childComponent.children && childComponent.children.size > 0) {
            // custom lwc under a block that is directly under a step
            childComponent.children.forEach(childInBlock => {
              lwcVariableList = lwcVariableList.concat(getCustomLwcAttributes(childInBlock));
            })
          }
          else {
            lwcVariableList = lwcVariableList.concat(getCustomLwcAttributes(childComponent));
          }
        })
      }
    });
  ...
  function getCustomLwcAttributes(component) {
    const lwcVariableList = [];
    if (component.custom && component.custom.varMapAttributes) {
      const keys = Object.keys(component.custom.varMapAttributes);
      keys.forEach(key => {
        let jsonDefRespVar = component.custom.varMapAttributes[key];
        if (jsonDefRespVar) {
          lwcVariableList.push(jsonDefRespVar.trim());
        }
      })
    }
    return lwcVariableList;
  }
  ...
  function separateEditBlockTemplate(compChildren, newPath, childrenPath, eleArrayPath) {
    ...
    if (!editBlockTemplates.editBlockReplace) {
      editBlockTemplates.editBlockReplace = {};
    }
    ...
    let sldsTemplate = editBlockTemplates.editBlockSldsTemplate + '';
    ...
    // nds
    let ndsTemplate = editBlockTemplates.editBlockNdsTemplate + '';
    ...
    let uniqueName = 'OMNICOMPILEREDITBLOCK' + Object.keys(editBlockTemplates.editBlockReplace).length;

    // store editblock markup globally then substitute back to correct template layout
    editBlockTemplates.editBlockReplace[uniqueName] = {
      slds: sldsTemplate,
      nds: ndsTemplate
    };
...
module.exports = OmniscriptHtmlBuilder;

// This is required for the OmniScript Preview to work
if (typeof window !== "undefined") {
  window.OmniscriptHtmlBuilder = OmniscriptHtmlBuilder;
}

},{"../compilerConfig.js":8,"../templates/coreruntime/cssTemplate":9,"../templates/coreruntime/htmlTemplate":10,"../templates/deactivated/cssTemplate":12,"../templates/deactivated/htmlTemplate":13,"../templates/managed/cssTemplate":16,"../templates/managed/editBlockTemplate":17,"../templates/managed/ndsCssTemplate":19,"../templates/managed/ndsTemplate":20,"../templates/managed/sldsTemplate":21}],5:[function(require,module,exports){
const OmniscriptLwcJavascriptTemplate = require('../templates/managed/jsTemplate.js');
const DeactivatedOmniscriptLwcJavascriptTemplate = require('../templates/deactivated/jsTemplate.js');
const CoreRuntimeOmniscriptLwcJavascriptTemplate = require('../templates/coreruntime/jsTemplate.js');

function OmniscriptLwcJsBuilder(namespacePrefix) {
    const maxFileLength = 100000;
     ...
     * Returns the JavaScript for the deactivated LWC componemnt
     */
    this.getCoreRuntimeJs = function (lwcName) {
        return CoreRuntimeOmniscriptLwcJavascriptTemplate
            .replace(/\{\$namespace\}/g, namespacePrefix)
            .replace(/\{\$lwcName\}/g, lwcName);
    }
    ...
    this.getOmniDefinitionFiles = function (lwcName, json) {
        ...
        if (jsonString.length > maxFileLength) {
            ...
            // Split the json definition into chunks of max file length
            const files = chunkSubstr(btoa(encodeURIComponent(jsonString)), maxFileLength)
                .map((text, idx) => {
                    return {
                        name: 'lwc/' + lwcName + '/omnidef_chunk' + idx + '.js',
                        contents: `export const chunk${idx} = "${text}"; `
                    }
                });
            ...
            for (let i = 0; i < files.length; i++) {
                imports += `import { chunk${i} } from './omnidef_chunk${i}.js';\n`;
                joins += `def += chunk${i};\n`;
            }
...
module.exports = OmniscriptLwcJsBuilder;

},{"../templates/coreruntime/jsTemplate.js":11,"../templates/deactivated/jsTemplate.js":14,"../templates/managed/jsTemplate.js":18}],6:[function(require,module,exports){
const OmniscriptLwcIconTemplate = require('../templates/iconTemplate.js');
...
function OmniscriptLwcMetaBuilder() {
    ...
    this.getPackageMetaXml = function (type, subType, language, sId, hidden, addRuntimeNamespace, namespace) {
        ...
        let namespaceMeta = '';
        let targets = `
            ${this.getTargetsAsXml()}
            <targetConfigs>
                ${this.getTargetConfigsAsXml()}
            </targetConfigs>`;
        ...
        <LightningComponentBundle xmlns="http://soap.sforce.com/2006/04/metadata">
            ...
            <description>${this.getDescription(sId)}</description>
            <isExposed>true</isExposed>${namespaceMeta}
            <masterLabel>${this.getMasterLabel(type, subType, language)}</masterLabel>${targets}
        </LightningComponentBundle>`;
    };

    this.getPackageIcon = function () {
        return OmniscriptLwcIconTemplate;
    }

    this.getPackageMetaXmlAsJson = function (type, subType, language, sId, hidden, addRuntimeNamespace, namespace) {
        type = type.replace(/[\W]+/g, '');
        subType = subType.replace(/[\W]+/g, '');

        let metaJsonObj = {
            apiVersion: this.getApiVersion(),
            description: this.getDescription(sId),
            isExposed: this.getIsExposed(),
            masterLabel: this.getMasterLabel(type, subType, language),
            targets: this.getTargetsAsJson(),
            targetConfigs: this.getTargetConfigsAsXml(type, subType)
        };

        if (addRuntimeNamespace === true || addRuntimeNamespace === "true") {
            metaJsonObj.runtimeNamespace = namespace;
        }

        return metaJsonObj;
    }

    this.getTargetsAsXml = function () {
        return `
        <targets>
    ...
    this.getTargetsAsJson = function () {
        return {
            target: [
    ...
    this.getTargetConfigsAsXml = function (type, subType) {
        return `
    ...
    this.getDescription = function (sId) {
        return `Vlocity OmniScript Auto-generated - ${sId}`;
    }
    ...
    this.getMasterLabel = function (type, subType, language) {
        return `${type}/${subType}/${language}`
    }
}

module.exports = OmniscriptLwcMetaBuilder;

},{"../templates/iconTemplate.js":15}],7:[function(require,module,exports){
const OmniscriptHtmlBuilder = require('./builders/omniHtmlBuilder.js');
const OmniscriptLwcMetaBuilder = require('./builders/omniMetaBuilder.js');
const OmniscriptLwcJsBuilder = require('./builders/omniJsBuilder.js');
...
function OmniscriptLwcCompiler(namespacePrefix) {
    ...
    this.compileActivated = function (lwcName, json, hidden, addRuntimeNamespace, namespace) {
        return new Promise((resolve, reject) => {
            ...
            return htmlBuilder.getHtml(json)
                .then(html => {
                    const resources = [];
    ...
    this.compileForCoreRuntime = function (lwcName, type, subType, language, sId, addRuntimeNamespace, namespace) {
        return new Promise((resolve, reject) => {
            const resources = [];

            // Get the HTML
            const html = htmlBuilder.getCoreRuntimeHtml(type, subType, language);
            resources.push({ name: `lwc/${lwcName}/${lwcName}.html`, source: html.html, type: 'html' });
            resources.push({ name: `lwc/${lwcName}/${lwcName}_nds.html`, source: "<template></template>", type: 'html' });
            resources.push({ name: `lwc/${lwcName}/${lwcName}.css`, source: html.css, type: 'css' });
            resources.push({ name: `lwc/${lwcName}/${lwcName}_nds.css`, source: html.css, type: 'css' });

            // Get the Javascript file
            resources.push({ name: `lwc/${lwcName}/${lwcName}.js`, source: jsBuilder.getCoreRuntimeJs(lwcName), type: 'js' });
    ...
    this.getPackageMetaXmlAsJson = function (type, subType, language, sId, lwcName, addRuntimeNamespace, namespace) {
        return metaBuilder.getPackageMetaXmlAsJson(type, subType, language, sId, lwcName, addRuntimeNamespace, namespace);
    }

    this.getZipFileMetadata = function () {
        return `<?xml version="1.0" encoding="UTF-8"?>
    ...
    * @returns {string} The LWC component name
    */
    this.getLwcName = function (type, subType, language) {
    ...
    * @returns {string} The tag for the LWC component. For example `c-type-Subtype-Language`
    */
    this.getComponentTag = function (lwcName) {
...
},{"./builders/omniHtmlBuilder.js":4,"./builders/omniJsBuilder.js":5,"./builders/omniMetaBuilder.js":6}],8:[function(require,module,exports){
const OmniscriptLwcCompilerConfig = {
    baseAttributes: {
        key: '{$object.lwcId}',
        'json-def': '{$object}',
    ...
    elements: {
        ...
        'PDF Action': {
            bProperty: 'bPDFAction',
            htmlTag: 'omniscript-pdf-action',
        },
        'Set Errors': {
...
module.exports = OmniscriptLwcCompilerConfig;
},{}],9:[function(require,module,exports){
const CoreRuntimeOmniscriptLwcCssTemplate = `/* Intentionally empty css */`;

module.exports = CoreRuntimeOmniscriptLwcCssTemplate; 

},{}],10:[function(require,module,exports){
const CoreRuntimeOmniscriptLwcHtmlTemplate = `<template>
    <{$namespace}-omnistudio-standard-runtime-wrapper
        ...
        direction={dir}
        record-id={recordId}
        omniscript-id={omniscriptId}
        run-mode={runMode}></{$namespace}-omnistudio-standard-runtime-wrapper>
</template>`;

module.exports = CoreRuntimeOmniscriptLwcHtmlTemplate;

},{}],11:[function(require,module,exports){
const CoreRuntimeOmniscriptLwcJavascriptTemplate = `
...
export default class {$lwcName} extends LightningElement {
    ...
    get runtimeWrapper() {
        return this.template.querySelector('{$namespace}-omniscript-standard-runtime-wrapper')?.generatedOs;
    }
}`;

module.exports = CoreRuntimeOmniscriptLwcJavascriptTemplate;

},{}],12:[function(require,module,exports){
const DeactivatedOmniscriptLwcCssTemplate = `
.footer-message {
    font-style: italic;
    font-weight: 700;
    margin-top: 10px;
...
module.exports = DeactivatedOmniscriptLwcCssTemplate;
},{}],13:[function(require,module,exports){
const DeactivatedOmniscriptLwcHtmlTemplate = `<template>
<div>
...
module.exports = DeactivatedOmniscriptLwcHtmlTemplate;
},{}],14:[function(require,module,exports){
const DeactivatedOmniscriptLwcJavascriptTemplate = `
import { api, LightningElement } from 'lwc';
...
module.exports = DeactivatedOmniscriptLwcJavascriptTemplate;
},{}],15:[function(require,module,exports){
const OmniscriptLwcIconTemplate = `<?xml version="1.0" encoding="UTF-8" standalone="no"?>
...
module.exports = OmniscriptLwcIconTemplate;
...
*/
const OmniscriptLwcCssTemplate = `
...
`;
module.exports = OmniscriptLwcCssTemplate;
},{}],17:[function(require,module,exports){
const editBlockSldsTemplate = `
    <{$namespace}-omniscript-edit-block-wrapper key={$object.lwcId} class={$object.sldsCls} json-def={$object} layout={layout} script-header-def={scriptHeaderDef}>
        $EDITBLOCKLABEL
        $EDITBLOCK
        $EDITBLOCKNEW
    </{$namespace}-omniscript-edit-block-wrapper>
`;

const editBlockSldsTemplateCls = "slds-grid slds-wrap slds-size_1-of-1 slds-is-relative omni-edit-block slds-size_$CTRLWIDTH-of-12";

const editBlockNdsTemplate = `
    <{$namespace}-omniscript-edit-block-wrapper key={$object.lwcId} class={$object.ndsCls} json-def={$object} layout={layout} script-header-def={scriptHeaderDef}>
        $EDITBLOCKLABEL
        $EDITBLOCK
        $EDITBLOCKNEW
    </{$namespace}-omniscript-edit-block-wrapper>
`;
const editBlockNdsTemplateCls = "nds-grid nds-wrap nds-size_1-of-1 nds-is-relative omni-edit-block nds-size_$CTRLWIDTH-of-12";

module.exports = {
    editBlockSldsTemplate: editBlockSldsTemplate,
    editBlockSldsTemplateCls: editBlockSldsTemplateCls,
    editBlockNdsTemplate: editBlockNdsTemplate,
    editBlockNdsTemplateCls: editBlockNdsTemplateCls
}
},{}],18:[function(require,module,exports){
const OmniscriptLwcJavascriptTemplate = `
import { api, track } from 'lwc';
import OmniscriptHeader from '{$namespace}/omniscriptHeader';
import { isOffPlatform, setConnection, getConnection } from '{$namespace}/omniscriptRestApi';
import { RUN_MODES, registerNs } from '{$namespace}/omniscriptInternalUtils';
...
module.exports = OmniscriptLwcJavascriptTemplate;
...
*/
const OmniscriptLwcNdsCssTemplate = `
...
`;
module.exports = OmniscriptLwcNdsCssTemplate;
},{}],20:[function(require,module,exports){
const OmniscriptLwcNdsTemplate = `
<template>
    <div dir={_dir}>
...
module.exports = OmniscriptLwcNdsTemplate;
},{}],21:[function(require,module,exports){
const OmniscriptLwcSldsTemplate = `
<template>
    <div dir={_dir}>
        <template if:true={hasErrors}>
            <lightning-layout vertical-align="stretch">
...
module.exports = OmniscriptLwcSldsTemplate;
},{}],22:[function(require,module,exports){

function ToolingClient(httpClient) {
    ...
    this.retrieveLwcComponent = retrieveLwcComponent;
    ...
    const urlPrefix = '/services/data/v56.0/tooling/',
    ...
    function existsLwcComponent(name) {
        const q = encodeURIComponent("SELECT Id, DeveloperName FROM LightningComponentBundle WHERE DeveloperName = '" + name + "'");
        return new Promise((resolve, reject) => {
    ...
    function removeLwcComponent(id) {
        return new Promise((resolve, reject) => {
            return submitRequest({
                url: sobjectsUrl + 'LightningComponentBundle/' + id,
                data: undefined,
                method: 'DELETE'
            }).then(result => {
                resolve(true);
    ...
    function deploy(stream) {
        return Promise.reject('This method has been deprecated.');
    }
    ...
    function retrieveLwcComponent(lwcName) {
        return new Promise((resolve, reject) => {
            submitRequest({
                url: urlPrefix + 'composite',
                data: JSON.stringify({
                    "allOrNone": true,
                    "compositeRequest": [
                        {
                            "method": "GET",
                            "url": "/services/data/v56.0/tooling/query/?q=SELECT%20Id%2CDeveloperName%2CDescription%20FROM%20LightningComponentBundle%20WHERE%20DeveloperName%20%3D%20%27" + lwcName + "%27",
                            "referenceId": "bundleInfo"
                        },
                        {

                            "method": "GET",
                            "url": "/services/data/v56.0/tooling/query?q=SELECT%20Id%2CFilePath%2CFormat%2CSource%20FROM%20LightningComponentResource%20WHERE%20LightningComponentBundleId%20=%20'@{bundleInfo.records[0].Id}'",
                ...
                .then(response => {
                    const resources = [];
                    const responseResources = response.compositeResponse.find(cr => cr.referenceId === 'bundleResources');
                    if (responseResources) {
                        const records = responseResources.body.records || [];
                        for (let i = 0; i < records.length; i++) {
                            resources.push({
                                name: records[i].FilePath,
                                source: records[i].Source,
                                type: records[i].Format,
                            })
                        }
                    }
                    resolve(resources);
     ...
     */
    function deployResources(lwcName, resources, sId, metaXmlJson) {
        return new Promise((resolve, reject) => {
            getLwcComponentBundle(lwcName)
                .then(bundle => usePatchMethod ?
                    processByPatch(bundle, lwcName, resources, sId, metaXmlJson) :
                    processByReplace(bundle, lwcName, resources, sId, metaXmlJson)
                )
                .then(resolve)
                .catch(reject)
        });
    ...
    function processByReplace(bundle, lwcName, resources, sId, metaXmlJson) {
        const promise = bundle ?
            updateLwcBundle(bundle.Id, sId, resources, metaXmlJson)
            ...
            :
            createLwcBundle(lwcName, sId, resources, metaXmlJson);
    ...
    function processByPatch(bundle, lwcName, resources, sId, metaXmlJson) {
     ...
     */
    function createLwcBundle(lwcName, sId, resources, metaXmlJson) {
        if (typeof resources == 'undefined') {
            return new Promise((resolve, reject) => {
                resolve();
            })
        }
        const lwcRes = resources.filter((res) => {
            return res.type != 'meta';
        }).map((res) => {
            return {
                filePath: res.name,
                source: btoa(unescape(encodeURIComponent(res.source)))
            }
        });
        return new Promise((resolve, reject) => {
            ...
            submitRequest({
                url: `${sobjectsUrl}LightningComponentBundle`, data: {
                    FullName: lwcName,
                    Metadata: getMetadata(lwcRes, metaXmlJson)
                }
            })
                .then(result => {
                    if (!result.success) {
                ...
                .catch(error => {
                    if (!Array.isArray(error)) error = [error];
                    if(error[0]?.data?.[0]?.errorCode === 'FIELD_INTEGRITY_EXCEPTION') {
                        reject(error[0].data[0].message);
                    }
                    reject([...new Set(error.map(e => e.message ? e.message : (typeof e === 'string' ? e : JSON.stringify(e))))]);
                })
        });
    }

    function updateLwcBundle(bundleId, sId, resources, metaXmlJson) {
    ...
    function getMetadata(lwcRes, metaXmlJson) {
        const metadata = {
            apiVersion: metaXmlJson.apiVersion,
            isExposed: metaXmlJson.isExposed,
            description: metaXmlJson.description,
            targetConfigs: btoa(metaXmlJson.targetConfigs),
            targets: metaXmlJson.targets,
            masterLabel: metaXmlJson.masterLabel,
            lwcResources: {
                lwcResource: lwcRes
            }
        };
        if (metaXmlJson.hasOwnProperty('runtimeNamespace') && metaXmlJson.runtimeNamespace) {
            metadata.runtimeNamespace = metaXmlJson.runtimeNamespace
        }
    ...
    function getBundleResources(bundleId) {
        const q = encodeURIComponent(`Select Id, FilePath From LightningComponentResource Where LightningComponentBundleId='${bundleId}'`);
        return new Promise((resolve, reject) => {
            return submitRequest({
                url: query + q,
                data: undefined,
    ...
    /**
     * A common error handler for toolingClient.
     * @param {any} reason - A promise result containing the reason  for rejection in the promise chain.
     */
    function handleError(reason) {
            if (reason?.data?.length) {
                reason = reason.data;
            }
            ...
            return [...new Set(reason.map(e => e.message))];
    }
}
...
const fieldMappingUtils = require('../../oui/util/fieldsMappingUtils');
...
(function () {
    ...
    window.angular.module('omniscriptLwcCompiler')
        .service('bpService', function () {
            ...
            if (window?.omniLwcCompilerConfig?.remoteActions) {
                let namespaceDot = window.omniLwcCompilerConfig.namespacePrefix + '.';
                let compilerRemoteActions = {
                    deployResources: {
                        action: namespaceDot + 'OmniScriptDesignerController.deployResources',
                        config: {escape: false,  buffer: false}
                    },
                    existsLwcComponent: {
                        action: namespaceDot + 'OmniScriptDesignerController.existsLwcComponent',
                        config: {escape: false,  buffer: false}
                    },
                    fetchLwcResources: {
                        action: namespaceDot + 'OmniScriptDesignerController.fetchLwcResources',
                        config: {escape: false,  buffer: false}
                    },
                    getLWCByDeveloperName: {
                        action: namespaceDot + 'OmniScriptDesignerController.getLWCByDeveloperName',
                        config: {escape: false,  buffer: false}
                    },
                    getNonOmniScriptLwcs: {
                        action: namespaceDot + 'OmniScriptDesignerController.getNonOmniScriptLwcs',
                        config: {escape: false,  buffer: false}
                    }
                };
                Object.assign(window.omniLwcCompilerConfig.remoteActions, compilerRemoteActions); 
            }

            this.loadActiveLwc = loadActiveLwc;
            this.loadInactiveLwc = loadInactiveLwc;
            this.replaceAllFields = replaceAllFields;
            this.deployResources = deployResources;
            this.existsLwcComponent = existsLwcComponent;
            this.fetchLwcResources = fetchLwcResources;
            this.getLWCByDeveloperName = getLWCByDeveloperName;
            this.getNonOmniScriptLwcs = getNonOmniScriptLwcs;

            function loadActiveLwc(type, subType, language, isFileBasedScript) {
                if (isFileBasedScript) {
                    // skip the below call for file based
                    return Promise.resolve({
                        bpType: type,
                        bpSubType: subType,
                        bpLang: language,
                        sOmniScriptId: `${type}_${subType}_${language}`
                    });
                }
                return new Promise((resolve, reject) => {
            ...
            /**
             * A wrapper to the replaceAllFields method
             * @param {String} json
             * @param {Boolean} beforeSave
             * @param {Boolean} isFoundation
             */
            function replaceAllFields(json, beforeSave, isFoundation) {
                return fieldMappingUtils.replaceAllFields(json, true, beforeSave, isFoundation);
            }
            ...
            function deployResources(resources) {
                return new Promise((resolve, reject) => {
                    Visualforce.remoting.Manager.invokeAction(
                        omniLwcCompilerConfig.remoteActions.deployResources.action,
                        resources,
                        json => {resolve(json)},
                        omniLwcCompilerConfig.remoteActions.deployResources.config,
                    );
                });
            }

            function existsLwcComponent(lwcName) {
                return new Promise((resolve, reject) => {
                    Visualforce.remoting.Manager.invokeAction(
                        omniLwcCompilerConfig.remoteActions.existsLwcComponent.action,
                        lwcName,
                        json => {resolve(json)},
                        omniLwcCompilerConfig.remoteActions.existsLwcComponent.config,
                    );
                });
            }

            function fetchLwcResources(lwcName) {
                return new Promise((resolve, reject) => {
                    Visualforce.remoting.Manager.invokeAction(
                        omniLwcCompilerConfig.remoteActions.fetchLwcResources.action,
                        lwcName,
                        json => {resolve(json)},
                        omniLwcCompilerConfig.remoteActions.fetchLwcResources.config,
                    );
                });
            }

            function getLWCByDeveloperName(lwcName) {
                return new Promise((resolve, reject) => {
                    Visualforce.remoting.Manager.invokeAction(
                        omniLwcCompilerConfig.remoteActions.getLWCByDeveloperName.action,
                        lwcName,
                        json => {resolve(json)},
                        omniLwcCompilerConfig.remoteActions.getLWCByDeveloperName.config,
                    );
                });
            }

            function getNonOmniScriptLwcs(lwcName) {
                return new Promise((resolve, reject) => {
                    Visualforce.remoting.Manager.invokeAction(
                        omniLwcCompilerConfig.remoteActions.getNonOmniScriptLwcs.action,
                        lwcName,
                        json => {resolve(json)},
                        omniLwcCompilerConfig.remoteActions.getNonOmniScriptLwcs.config,
                    );
                });
            }
        });
}());
},{"../../oui/util/fieldsMappingUtils":28}],25:[function(require,module,exports){
const OmniscriptLwcCompiler = require('../compiler/lwc/compiler');

(function () {
    /* jshint -W030 */
    'use strict';
    angular.module('omniscriptLwcCompiler')
        .service('compilerService', ['bpService', '$modal', '$rootScope', function (bpService, $modal, $rootScope) {
            ...
            this.getComponentZipFromOrgMetadata = getComponentZipFromOrgMetadata;
            this.compileActivated = compileActivated;
            this.showDeploymentError = showDeploymentError;
            this.getLwcErrors = getLwcErrors;
            this.getPackageMetaXmlAsJson = getPackageMetaXmlAsJson;
            this.getComponentZipFromOrgMetadataForOffPlatform = getComponentZipFromOrgMetadataForOffPlatform;
            this.getZipFileMetadata = getZipFileMetadata;
             ...
             * @param {string} lwcName
            */
            function getComponentZipFromOrgMetadata(lwcName) {
                return new Promise((resolve, reject) => {
                    bpService.fetchLwcResources(lwcName)
                        .then(resources => {
                            if (resources.length == 0) {
                                reject('No resources found for LWC component');
                            };
                            const zip = new JSZip();
                            zip.file('package.xml', compiler.getZipFileMetadata());
                            resources.forEach(resource => {
                                zip.file(resource.name, resource.source);
                            });
            ...
            function getComponentNameAndNamespace(componentName) {
                let namespace = 'c',
                    name = componentName;
            ...
            function getComponentZipFromOrgMetadataForOffPlatform(jsonDef, lwcName, namespacePrefix, isInsidePckg) {
                const componentsToDownload = new Map();

                // OmniOut notes:
                // If we are in standard runtime, we always compile the LWC again as we might not have an LWC in the
                // org, or, if we have an LWC, it might be referencing the stanrdard runtime wrapper instead of the
                // full LWC component we need for OmniOut.
                if (!IsStandardRuntime) {
                    componentsToDownload.set(lwcName, {
                        name: lwcName,
                        namespace: 'vlocityomniscript'
                    })
                }

                // Get the custom components from the header
                if (jsonDef.propSetMap && jsonDef.propSetMap.elementTypeToLwcTemplateMapping) {
                    const headerKeys = Object.keys(jsonDef.propSetMap.elementTypeToLwcTemplateMapping);
                    headerKeys.forEach(key => {
                        if (jsonDef.propSetMap.elementTypeToLwcTemplateMapping.hasOwnProperty(key)) {
                            let name = jsonDef.propSetMap.elementTypeToLwcTemplateMapping[key];
                            componentsToDownload.set(name, getComponentNameAndNamespace(name));
                        }
                    });
                }

                // Now, the custom components from the steps
                jsonDef.children.forEach(child => {
                    if (child.type === 'Step') {

                        if (child.propSetMap && child.propSetMap.lwcComponentOverride) {
                            let name = child.propSetMap.lwcComponentOverride;
                            componentsToDownload.set(name, getComponentNameAndNamespace(name));
                        }

                        getCustomComponents(child.children || [], componentsToDownload);
                    }
                });
                ...
                return new Promise((resolve, reject) => {
                    ...
                    const promises = new Array(...componentsToDownload.keys())
                        .filter(name => {
                            // If packaged component, skip it
                            if (isInsidePckg && name.startsWith(namespacePrefix + '__')) return false;
                            return !!String(name).trim();
                        })
                        .map(name => bpService.fetchLwcResources(name).then(resources => Promise.resolve({ ...componentsToDownload.get(name), resources })));

                    if (IsStandardRuntime) {
                        promises.unshift(new Promise(resolve => {
                            compiler.compileActivated(lwcName, jsonDef, false, isInsidePckg, namespacePrefix)
                                .then(resources => {
                                    resolve({
                                        name: lwcName,
                                        namespace: 'vlocityomniscript',
                    ...
                    Promise.all(promises)
                        .then(results => {
                            ...
                            // Throw an error is we don't have results on the first element (the main LWC)
                            if (results.length === 0) {
                                reject('No LWC component was found');
                            };

                            // Create the zip file
                            const zip = new JSZip();

                            // Loop into each result (each component)
                            results.forEach(component => {

                                // Throw an error is we don't have results on the first element (the main LWC)
                                if (component.resources.length === 0) {
                                    reject('Could not find LWC component');
                                };

                                // Add to the zip file
                                component.resources.forEach(resource => {

                                    const resourceName = resource.name.replace('lwc/', component.namespace + '/');
                                    let resourceSource = resource.source;

                                    // skip namespace replacement for jsonDef files
                                    if (resource.name.startsWith('jsonDef') || resource.name.endsWith('_def.js')) {
                                        resourceSource = resource.source;
                                    }
                                    else {
                                        resourceSource = resource.source.replace(new RegExp(namespacePrefix, 'g'), 'c');
                                    }

                                    // Replace the namespace with 'c' namespace
                                    zip.file('vlocityomniout/src/modules/' + resourceName, resourceSource);
                                });

                            });

                            // Generate the stream and resolve
                            zip.generateAsync({ type: "blob" })
                                .then(function (content) {
                                    resolve(content)
                                });
            ...
            function getLwcName(type, subType, language) {
                return compiler.getLwcName(type, subType, language);
            }
             ...
             */
            function compileActivated(lwcName, json, addRuntimeNamespace, namespace, IsStandardRuntime, backcompat) {
                if (IsStandardRuntime) {
                    return new Promise((resolve, reject) => {
                        // TODO
                        bpService.existsLwcComponent(lwcName)
                            .then(exists => {
                                if (exists || backcompat) {
                                    compiler.compileForCoreRuntime(lwcName, json.bpType, json.bpSubType, json.bpLang, json.sOmniScriptId, addRuntimeNamespace, namespace)
                                        .then(resources => {
                                            resources?.push({ name: "package.xml", source: compiler.getZipFileMetadata() });
                                            bpService.deployResources(resources);
                                        })
                                        .then(resolve)
                                        .catch(reject);
                                } else {
            ...
            function getPackageMetaXmlAsJson(type, subType, language, sId, lwcName, addRuntimeNamespace, namespace) {
                return compiler.getPackageMetaXmlAsJson(type, subType, language, sId, lwcName, addRuntimeNamespace, namespace);
            }

            function getZipFileMetadata() {
                return compiler.getZipFileMetadata();
            }


            function verifyIfExists(lwcName) {
                return bpService.existsLwcComponent(lwcName);
            }
            ...
            function getComponentTag(lwcName) {
                return compiler.getComponentTag(lwcName);
            }
            ...
            /**
             * Loops into the json definition children and finds all custom LWC components
             * @param {Array} children
             * @param {Map} customComponents
             */
            function getCustomComponents(children, customComponents) {

                children.forEach(child => {
                    child.eleArray.forEach(elem => {

                        let names = [];
                        if (elem.type === 'Custom Lightning Web Component' && elem.propSetMap.lwcName) {
                            names.push(elem.propSetMap.lwcName);
                        } else if (elem.propSetMap && elem.propSetMap.lwcComponentOverride) {
                            names.push(elem.propSetMap.lwcComponentOverride);
                        }

                        // handle edit block's lwc overrides
                        if (elem.propSetMap && elem.type === 'Edit Block') {
                            if (elem.propSetMap.editBlockNewLwcOverride) {
                                names.push(elem.propSetMap.editBlockNewLwcOverride);
                            }
                            if (elem.propSetMap.editBlockLabelLwcOverride) {
                                names.push(elem.propSetMap.editBlockLabelLwcOverride);
                            }
                        }

                        if (names.length > 0) {
                            names.forEach(lwcname => {
                                customComponents.set(lwcname, getComponentNameAndNamespace(lwcname));
                            })
                        }

                        // Loop into the children if exists
                        if (elem.children && Array.isArray(elem.children)) {
                            getCustomComponents(elem.children, customComponents);
                        }
                    });
                })
            }
...
},{"../compiler/lwc/compiler":7}],26:[function(require,module,exports){
(function () {
    const toolingUtils = require('../compiler/tooling/toolingClient');
    ...
    window.angular.module('omniscriptLwcCompiler')
        .service('toolingService', ['$http', function ($http) {
            ...
            const toolingClient = new toolingUtils(httpClient);

            // All modules have been moved into a tooling utils file. 
            // This in order to allow using the utility in external components.
            this.existsLwcComponent = toolingClient.existsLwcComponent;
            this.getLwcComponentBundle = toolingClient.getLwcComponentBundle;
            this.removeLwcComponent = toolingClient.removeLwcComponent;
            this.deploy = toolingClient.deploy;
            this.deployResources = toolingClient.deployResources;
            this.getNonOmniScriptLwcs = toolingClient.getNonOmniScriptLwcs;
            this.retrieveLwcComponent = toolingClient.retrieveLwcComponent;

        }]);
}());

},{"../compiler/tooling/toolingClient":22}],27:[function(require,module,exports){
(function () {
    /* jshint -W030 */
    'use strict';
    angular.module('omniscriptLwcCompiler').controller('vfPageController', function ($rootScope, $scope, compilerService, bpService) {
